...
| Intake one Papaya event | `ppy intake [event.json\|-]` — reads `PAPAYA_EVENT_FILE` by default, writes missing acceptance criteria to the tracked record first, starts the supervisor when needed, registers and onboards the named repository through the normal account/organisation boundary, and dispatches exactly once per event identity. A local checkout is used only to read its `origin`; work always happens in the runtime-owned clone. The command exits after dispatch. It does not own the Papaya subject reservation: the listener that invokes it must retain and renew that reservation until task delivery rather than releasing it when this short-lived child exits. |
