...
- **Nothing to build parks the ticket.** A brief turn ending `NOTHING TO BUILD:`
  ...
  it). `ppy sweep --include-declined` offers it by hand; any pickup forgets it.
- **Repetition is a deficiency, once.** A work item picked up three times inside an
  hour with no phase beyond the brief, or refused here for the same reason on three
  sweeps running, records `repeated-without-progress`, fingerprinted on the ticket
  and how it ended (`reported`, `declined`, `stalled`, `released`, or the refusal
  code). The same fingerprint is recorded at most once a day, however long the
  repetition goes on.
- **`needs attention`** closes `ppy workers` and `ppy status --team`, one line each:
  ...
  worst first, with its count and last-seen time. A look is rendering either command
  (not `--json`); the counts it saw are kept in `.ppy/attention-seen.json`, and a
  first look counts every row from one. Say these lines to the person; do not let one
  sit.
