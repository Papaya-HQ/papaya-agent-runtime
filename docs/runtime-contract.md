...
| Capability requests | A worker names a program it needs in its plan phase: `ppy need <task> --capability <program> --why "..."`; a plain command its profile refused becomes the same request. This machine's policy decides first — the safe family and `capabilities.auto_grant` are granted (the pattern joins `claude.extra_tools`), the never list and `capabilities.never` are refused — and anything else waits on a person as the `capability_request_pending` blocker, which the outreach procedure says to them (their DM with this agent, the work item, the desktop) and repeats until decided. `ppy capability list [--all] [--task <id>]`; `ppy capability approve <id>` grants the task alone (its next launch carries the pattern), `--always` grants every worker here; `ppy capability deny <id> --reason "..."`. The worker is steered with every outcome it did not ask to hear. `ppy config capabilities --auto-grant/--never/--remove <program>` edits the policy; no setting lowers the never list |
